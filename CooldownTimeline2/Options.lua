			...
			global = {
				...
				args = {
					...
					--[[dynamicSpellDetection = {
						...
						width = "full",
						get = function(info, index)
								return CDTL2.db.profile.global["dynamicSpellDetection"]
							end,
						set = function(info, val)
								if not CDTL2.db.profile.global["dynamicSpellDetection"] then
									CDTL2:ScanSpellbook()
								end
				...
				args = {
					...
					exportbox = {
						...
						get = function(info)
								...
								local serialized = LibAceSerializer:Serialize(CDTL2.db.profile)
					...
					importbox = {
						...
						set = function(info, val)
								-- DEFLATE
								local LibDeflate = LibStub:GetLibrary("LibDeflate")
								local unprintable = LibDeflate:DecodeForPrint(val)
								...
								if success then
									...
									CDTL2.db.profile.global = data.global
									CDTL2.db.profile.lanes = data.lanes
									CDTL2.db.profile.barFrames = data.barFrames
									CDTL2.db.profile.ready = data.ready
									CDTL2.db.profile.holders = data.holders
									CDTL2.db.profile.tables = data.tables

									CDTL2:RefreshConfig()	
									--CDTL2:Print("Data Imported?")
								else
									CDTL2:Print("There was an error when attemping to import settings")
								end
